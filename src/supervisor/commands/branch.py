...
from supervisor.aliases import (
    load_aliases,
    require_worktree,
    reverse_aliases,
    save_aliases,
)
from supervisor.git import git, git_passthrough, list_worktrees, unlock_worktree
...
from supervisor.resolution import active_context
from supervisor.summary import worktree_info
from supervisor.utils import bold, colorize, die, dim, group_by_date, render_date_tree, warn
...
def _sync(repo: str = "bag") -> None:
    ...
    from supervisor.paths import staging_worktree
...
def create_branch(
    ...
    """
    from supervisor.config import load_repos
    from supervisor.paths import prod_ref
    ...
    from supervisor.deploy_shared import remove_staging_worktree
    ...
    issue_id = None
    if not no_issue:
        from supervisor.linear_shared import create_issue, save_issue_link
...
def _teardown_docker_containers(qualified: str) -> None:
    ...
    """
    from supervisor.serve.docker import COMPOSE_FILE, compose_down, compose_project_name
...
def rename_branch(qualified: str, new_name: str) -> dict[str, str]:
    ...
    from supervisor.paths import STAGING_MERGE_STATE
    ...
    from supervisor.git import git as git_fn
