...
from supervisor.serve.github_tokens import _get_fernet
...
def get_token(username: str, provider: str, jwt_secret: str) -> str | None:
    ...
    if provider == "github":
        from supervisor.serve.github_tokens import get_github_token
...
def delete_token(username: str, provider: str) -> bool:
    ...
    with _lock:
        ...
        if provider not in tokens:
            ...
            if provider == "github":
                from supervisor.serve.github_tokens import delete_github_token
...
def has_token(username: str, provider: str) -> bool:
    ...
    if provider in tokens:
        return True
    if provider == "github":
        from supervisor.serve.github_tokens import has_github_token
