...
from supervisor.serve.dependencies import (
    get_error_log,
    get_event_manager,
...
from supervisor.serve.metrics import MetricsCollector
from supervisor.serve.operation_progress import OperationTracker
from supervisor.serve.ports import PortAllocator
from supervisor.serve.service_lifecycle import cleanup_branch_services, list_remaining_volumes, unregister_service_ports
from supervisor.serve.service_lifecycle import reinstall_deps as lifecycle_reinstall_deps
from supervisor.serve.service_lifecycle import setup_branch_services as lifecycle_setup
from supervisor.serve.service_lifecycle import start_service as lifecycle_start
from supervisor.serve.service_lifecycle import stop_service as lifecycle_stop
from supervisor.serve.services import ServiceInstance, ServiceManager, State
from supervisor.serve.templates import load_services_config, resolve_placeholders
from supervisor.serve.test_runner import discover_suites, run_tests, stop_test_run
...
# unreachable (see ``supervisor.inspect.resolver``).  Schema matches the
...
async def trigger_discover_services(
    ...
    ``supervisor.serve.discovery.discover_docker``) and registers any
    ...
    """
    from supervisor.serve.discovery import discover_docker
...
def write_vite_ports_state_from_registry() -> None:
    ...
    """
    # Local import to avoid an import cycle: supervisor.serve.services <-
    # this module <- supervisor.serve.events <- this module.
    from supervisor.serve.services import services as services_singleton
...
async def run_test_suite(req: TestRunRequest) -> object:
    from supervisor.serve.test_ops import resolve_test_env, resolve_test_file, tests_dir
