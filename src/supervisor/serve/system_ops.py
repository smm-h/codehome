...
from supervisor.serve.updater import _PROJECT_ROOT
...
def supervisor_disk_usage() -> int:
    ...
    from supervisor.paths import SUPERVISOR_DIR, superv_home
...
async def build_health_status() -> dict[str, Any]:
    ...
    """
    from supervisor.serve.events import events as _events
    from supervisor.serve.server import _server_start_time
    from supervisor.serve.services import State
    from supervisor.serve.services import services as _services
...
def get_diagnostics_stats() -> dict[str, Any]:
    ...
    from supervisor.serve.events import events as _events
    from supervisor.serve.middleware import request_count
    from supervisor.serve.server import _server_start_time
    from supervisor.serve.services import State
    from supervisor.serve.services import services as _services
...
def get_request_metrics() -> dict[str, Any]:
    ...
    """
    from supervisor.serve.middleware import request_history
...
async def shutdown_all_services() -> int:
    ...
    from supervisor.serve.events import events as _events
    from supervisor.serve.ports import ports as _ports_singleton
    from supervisor.serve.service_lifecycle import cleanup_branch_services
    from supervisor.serve.services import services as _services
...
def find_logo() -> Path | None:
    ...
    from supervisor.paths import resolve_global
...
async def stream_self_update() -> AsyncGenerator[str, None]:
    ...
    try:
        ok, output = await _run_step(
            [sys.executable, "-c", "from supervisor.serve.server import app; print('ok')"],
        )
        if not ok:
