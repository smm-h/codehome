...
under ``supervisor.commands.*``, ``supervisor.checks.*``, and
``supervisor.extensions.*`` to detect missing imports, stale references,
...
Runs the imports in-process since the supervisor package is already
...
def _import_extension_file(py_file: Path) -> str | None:
    ...
    ``supervisor.extensions.loader``.  Returns None on success or an
...
async def check_imports(ctx: CheckContext) -> CheckResult:
    """Import all supervisor core modules to catch errors."""
    src = ctx.root / "src"
