...
2. **orphaned-cmd-functions** -- scan all ``cmd_*`` function definitions
   in ``src/supervisor/commands/`` and flag any that are not wired into
...
def _module_to_path(module_path: str, src: Path) -> Path:
    ...
    Handles both flat modules (supervisor.commands.branch -> branch.py)
    and packages (supervisor.commands.publisher -> publisher/__init__.py).
    """
...
async def command_file_mismatch(ctx: CheckContext) -> CheckResult:
    ...
    cli_py = ctx.root / "src" / "supervisor" / "cli.py"
...
def _collect_cmd_definitions(commands_dir: Path) -> list[tuple[str, str]]:
    ...
    for py_file in sorted(commands_dir.rglob("*.py")):
        ...
        # e.g. .../src/supervisor/commands/branch.py -> supervisor.commands.branch
        # e.g. .../src/supervisor/commands/publisher/__init__.py -> supervisor.commands.publisher
...
def _package_key(module_path: str) -> str:
    """Return the top-level package under supervisor.commands for grouping.

    ``supervisor.commands.telemac.transport`` -> ``supervisor.commands.telemac``
    ``supervisor.commands.branch``            -> ``supervisor.commands.branch``

    Sub-module cmd_* handlers are called by their parent package's wired
    handler, so reachability must span the entire package.
    """
    prefix = "supervisor.commands."
...
def _collect_plugin_wired_refs(plugins_dir: Path) -> set[tuple[str, str]]:
    """Scan plugin handlers.py files for imports from supervisor.commands.

    Plugin handlers that delegate to ``supervisor.commands.*`` functions
    ...
    for handler_file in sorted(plugins_dir.glob("*/handlers.py")):
        ...
        for node in ast.walk(tree):
            # Match: from supervisor.commands.foo import bar
            if isinstance(node, ast.ImportFrom) and node.module and node.module.startswith("supervisor.commands."):
...
async def orphaned_cmd_functions(ctx: CheckContext) -> CheckResult:
    ...
    from ``supervisor.commands.*`` also count as wiring.
    """
    cli_py = ctx.root / "src" / "supervisor" / "cli.py"
    commands_dir = ctx.root / "src" / "supervisor" / "commands"
    ...
    # supervisor.commands.* functions (these are not orphans).
    ...
    for pkg, pkg_defs in sorted(defs_by_package.items()):
        ...
        # (e.g. supervisor.commands.telemac wired, sub-modules are not).
