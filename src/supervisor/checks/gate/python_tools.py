...
async def mypy_check(ctx: CheckContext) -> CheckResult:
    """Run mypy type-checking on the supervisor Python package."""
    rc, stdout, stderr = await run_command(
        ["uv", "run", "mypy", "src/supervisor/"],
        cwd=ctx.root,
    )
    if rc == 0:
...
async def pytest_check(ctx: CheckContext) -> CheckResult:
    """Run the supervisor test suite via pytest."""
    rc, stdout, stderr = await run_command(
        ["uv", "run", "pytest", "src/supervisor/", "--tb=short"],
        cwd=ctx.root,
    )
    if rc == 0:
