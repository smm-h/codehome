...
Extracted from codehome.linear so that core never imports the linear
plugin directly. The linear plugin (plugins/linear/) imports from here
via codehome.linear, which re-exports everything.
...
from codehome.utils import atomic_json_write, load_json
