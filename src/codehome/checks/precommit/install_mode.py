"""Install-mode check: detect editable vs frozen codehome installs.
...
- Else, if the codehome package __file__ lives under the source repo
  (ROOT/src/codehome), it is editable-by-path (legacy `pip install -e .`
...
def _detect_install_mode() -> tuple[str, str, Path]:
    ...
    """
    pkg_file = Path(codehome.__file__).resolve()
    ...
    # it).
    try:
        dist = importlib.metadata.distribution("codehome")
    ...
    src_supervisor = ROOT / "src" / "codehome"
    try:
...
async def check_install_mode(ctx: CheckContext) -> CheckResult:
    """Detect if codehome is installed in editable mode.
