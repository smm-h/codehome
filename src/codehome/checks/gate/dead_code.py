...
functions and classes in ``src/codehome/`` and ``repos/*/extensions/``
...
async def dead_code(ctx: CheckContext) -> CheckResult:
    """Find Python functions/classes with zero references in src/codehome/ and repos/*/extensions/."""
    supervisor_dir = ctx.root / "src" / "codehome"
    if not supervisor_dir.is_dir():
        return CheckResult(
            name="dead-code",
            outcome="fail",
            duration_ms=0,
            message=f"codehome package not found at {supervisor_dir}",
        )
    ...
    if not py_files:
        return CheckResult(
            ...
            outcome="fail",
            duration_ms=0,
            message="no .py files found in src/codehome/",
        )

    # src_root is the parent of src/codehome/ so relative paths
    # start with src/codehome/...
    src_root = ctx.root
