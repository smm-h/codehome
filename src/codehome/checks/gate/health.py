...
- ``_check_linear_vs_git`` is inlined below (depends on codehome core)
...
async def health_raw_components(ctx: CheckContext) -> CheckResult:
    ...
    """
    try:
        from codehome.checks.gate._raw_components import (
            _EXCLUDED_FILES,
            DASHBOARD_SRC,
            scan_file,
        )
...
async def health_routes(ctx: CheckContext) -> CheckResult:
    ...
    """
    try:
        from codehome.checks.gate._routes import (
            _ROUTES_FILE,
            _SRC_DIR,
            _discover_routes,
