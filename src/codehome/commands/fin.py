...
from codehome.linear_shared import auto_advance_state
...
def _is_merged_to_production(branch: str, repo: str) -> bool:
    from codehome.git import is_merged_to_production

    return is_merged_to_production(branch, repo)


def _is_merged_to_staging(branch: str, repo: str) -> bool:
    from codehome.git import is_merged_to_staging
...
def _archive_branch(repo: str, name: str, outcome: str, message: str | None, keep_worktree: bool = False) -> None:
    ...
    try:
        from codehome.linear_shared import get_linked_issue
    ...
    if not keep_worktree:
        ...
        from codehome.aliases import load_aliases, save_aliases
...
def close_branch(
    ...
    *,
    ...
    if not cancel and not force:
        from codehome.git import worktree_is_clean
...
def cmd_fin(args: argparse.Namespace) -> None:
    ...
    if not cancel and not force:
        from codehome.progress import latest_section_is_empty
